...
### 八、派出去的 agent 与会话，拿到结果就关

**实测过的残留形态**（2026-08-01 一次盘点里同时存在三种）：

- **具名子 agent 交付完不会自己退**。带 name 的 agent 是邮箱型的，
  SendMessage 交完报告后一直挂着等追问 —— 三轮测试 agent 全部空闲驻留。
  拿到报告就 `TaskStop <name>`，除非明确还要续问
- **子 agent 的收尾指令不能只靠嘱咐**。prompt 里写了「结束时
  `agent-browser close`」，实测仍有会话没关掉（agent 忘了或 close 失败）。
  派发方在收报告后自己 `agent-browser session list` 核对一遍，
  残留的补一刀 —— 只关自己这个会话名下的，别的会话（如并行项目的）不碰
- **等待器脚本的超时分支与「找到」共用退出码 0**，看起来完成了、
  实际目标还在跑，派发方从此失去跟踪。等待器要么只在找到时退出，
  要么用不同退出码区分超时
- codex 任务照 codex-collab 的铁律：收工跑一次
  `codex_ps.sh --count`，不为零就 `--kill-orphans`

判据是**盘点而不是记忆**：收尾时把
`agent-browser session list` / `codex_ps.sh --count` / 后台任务列表
各扫一眼，比「我记得都关了」可靠。
...
排期在 `docs/ROADMAP.md`，具体条目在 `docs/DEBT.md`。当前最该先修的三条：

| 顺序 | 事项                                         | 为什么排这里                                |
| ---- | -------------------------------------------- | ------------------------------------------- |
| 1    | B-8 余项：流式(H-2)/历史续接(H-4)/取消(H-7)  | H-1 会话续接已修，其余症状仍在那条链上      |
| 2    | 事件零发射余账（守卫已齐，白名单即现状清单） | 违反架构第一原则「RunEvent 是唯一事实来源」 |
| 3    | B-5 白名单字段在表单上标「引擎目前不读它」   | 填了不生效比报错更糟                        |
