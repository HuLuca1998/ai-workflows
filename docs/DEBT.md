...
**任务清单第 1 条的另一半（ACP 用户消息队列）查下去挖出来的。**

消息队列本身**已经做了而且是真的**（`apps/web/src/supervisor/messageQueue.ts`，
23 条测试，四种突变 —— 队尾出队 / 上限失效 / 撤回失效 / 超长不拦 —— 全部变红）。
真正的坑在旁边那个按钮上。

「取消」原来只做三件事：把一个自增号码 +1（让回来的答案被丢掉）、
`setBusy(false)`、在对话里留一句「已取消」。**agent 那边照说不误** ——

- 配额照烧（一轮真实对话 10 到 30 秒）
- 会话槽位照占，用户接着发的下一句要排在这一轮后面才轮得到
- 那一轮的工具调用照做，而主管 AI 手上有 51 个系统 MCP 工具，其中有写操作

`AcpClient::cancel` 早就存在（`acp.rs:488`），**零调用点**，
而且它自己也是错的：用 `self.request(…)` 发，带 `id`。

**实测两种形态（codex-acp 1.1.7，探针见提交历史）**：
...
差一个字段，一个是「什么都没发生」，一个是当场停。

**修法**：`session/cancel` 改成通知；`AcpClient` 出 `CancelHandle`（只借 stdin 锁，
不碰槽位锁 —— 槽位锁被 `prompt` 占着整轮，去 lock 只会排到这一轮自己结束）；
`SessionPool` 用一张 `running` 表登记正在跑的轮次；契约加 `supervisor.cancel`，
两个宿主与界面按钮都接上。

守卫：`crates/engine/tests/acp_cancel_test.rs`（4 条）、
`apps/web/tests/supervisor-cancel.test.tsx`（3 条），都突变验证过。

**这一轮的两条方法教训**：

1. **突变验证抓到了我自己写的假测试。** 给 `CancelHandle` 那份加回 `id`，
   四条测试**全绿** —— 因为取消的 JSON 有两处各拼一份，
   而测试只盖了 `AcpClient::cancel` 那条路径，界面要用的恰恰是另一条。
   收敛成一处之后突变才红。**同一件事写两遍，测试盖住一遍就够绿了**
2. **`prompt` 的超时是「每条消息」的，不是「整轮」的。**
   第一版测试用了一个说个不停的假 adapter，指望 5 秒超时兜底 ——
   结果 `recv_timeout` 每收到一条就重置，测试跑了 10 分钟没结束。
   这不只是测试的问题：**一个卡住但仍在输出的 agent 能把
...
### B-21 · 校验器一直在报，而我们自己的模板带着 21 条 warning 出厂（已修）

修完 B-20 顺手把校验器对内置模板报的东西全打出来看 —— **21 条
`PORT_NO_DOWNSTREAM`，八个模板一条不落**，全是 `failed` 端口没有下游。

三件事：

**一、那条警告的文案已经过时，而且在误导人。** 它写的是
「走到它们时运行会静默停下 —— 以「成功」结束而后面什么都没做」。
那是 O-27 修之前的行为。现在走到没下游的端口是**响亮的失败**，
事件里点名「停在「X」的 Y 端口上」。文案不改，用户读到的是一个
已经不存在的行为。两侧同步改成
「走到它们时运行就停在那里判失败（事件里会写明停在哪个节点的哪个端口）——
想让它有别的结局，把这个端口接到一个终点」。

**二、`github-issue-fix` 的 `notify.failed` 该接。** 别的三条带 notify 的
模板本来就接了（repo-digest / server-log-triage / pr-followup，
注释写着「通知发不出去不该把整份报告丢掉」），只有它漏。
实测后果：Web 形态下 devserver 没有桌面通知发送器，`notify` 如实走 failed，
于是 PR 明明建好了，整条运行收在「停在「系统通知」的 failed 端口上」。

**三、锁文件不该进 PR。** `push_pr` 的新文件筛选排的是 `node_modules/`
这类目录，而 `verifyCommands: ['pnpm test']` 在一个没有 lockfile 的仓库里
必然生成一个 `pnpm-lock.yaml` —— 它于是被当成「这次修复新增的文件」。
现在加一条排除，**只排未跟踪的**：仓库本来就跟踪锁文件的话，
`git add -u` 已经覆盖了它的改动，真的加了依赖的修复不会漏。

实跑验过那条 shell 正则（它容易写错，只看代码不算数）：
两个锁文件与 `node_modules/` 被排除，`src/cart.js` 进来了，
而 `docs/pnpm-lock.yaml.md` 与 `src/lockutil.js` 没有被误伤。

**这条留下的教训**：`PORT_NO_DOWNSTREAM` 这道门禁**一直在正确地报**，
21 条一条不漏 —— 只是从来没有人去看它对内置模板报了什么。
...
「adapter 有没有失联」，不是「这一轮会不会结束」。

一个持续输出但永不收尾的 agent（死循环、被抢占那一轮的异常路径、
adapter 自己的 bug）会让这条 `prompt` 永远返回不了。它占着：
devserver 的一个工作线程（共 16 个）、会话槽位锁、一个 adapter 子进程。

实测撞到过（写取消测试时）：假 adapter 每 50ms 说一句，`prompt` 挂了 10 分钟
没有任何超时。真实 agent 一般会自己收尾，所以至今没在生产里咬到 ——
但现在有取消了，这条的严重度反而降了（用户能脱身），只是**自动化路径
（AI 节点、子工作流）没有取消按钮**。

修法：再加一个「整轮墙钟上限」，与每条消息的超时并存。
