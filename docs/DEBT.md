...
**B-13 · AI 节点永远走第一个端口（安全相关）**

`executor.rs` 取 `outputs(...).first()`，注释自承「条件路由还没做」。
于是 `ai.analyze` 恒 `success`、`ai.review` 恒 `passed`、`ai.decide` 恒
`auto_decided`。端口路由修真之后，这从「多跑一个节点」变成
**「少一道人工门」**：`github-issue-fix` 的 `approve_diff`（push 前的
改动确认）三条入边全是非首端口，**现在永远不执行，`git push` +
`gh pr create` 直接跑**。

修它要实现条件路由（读 AI 输出决定端口），是一块新功能不是补丁。
在那之前，那几条模板的失败终点是装饰。

**其余中低条目**：取消父运行时线程不退且子运行照跑、子运行忽略自己
入口的固定工作目录、子运行的 `trigger_kind` 污染「上次自动运行」、
子运行拿不到流式输出与 mock adapter、`release-checklist` 在无 tag 时
静默产出空改动清单、`dep-upgrade-audit` 不带 `-r` 漏 40% 依赖、
三处无标记的 `--limit` 截断、分组行的无障碍名里没有工作流名、
`LaunchDialog` 把空串当成「键不存在」。
