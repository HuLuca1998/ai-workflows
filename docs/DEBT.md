...
- **子运行忽略自己入口的固定工作目录**：`run_subworkflow` 绕开了
  `run_start`，而那段「固定目录压过调用方」的规则只在 core-api 有
- **子运行的 `trigger_kind` 污染「上次自动运行」**：既配定时又被当
  子工作流调用的工作流，自己的定时会被子调用顶掉
- **子运行占着父运行的执行线程**，与 `supervisor.rs` 模块头
  「挂起在审批上的运行不占线程」的明文承诺相反
- **`${input.repo.branch}` 在 repo-digest 里填了不生效**：脚本只用
  `repo.name`，`gh api repos/X/commits` 走默认分支
- **`verifyCommands` 零消费**，而 issue-feature 的 summary 写着「→ 自测 →」
- **`approval.interaction` 不生效**（白名单已记）
- **人工拒批不走 `rejected` 端口**（O-24 已记）
