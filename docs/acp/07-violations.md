...
项目所有者的决定是**不再对 AI 设权限门槛，按最高权限运行**。
理由是**风险控制的粒度放在节点职责上，而不是 ACP 权限档上**——
工作流里每个节点只干最小的一件事，图是可见的，做了什么进事件流。
一道逐工具的确认框拦不住设计上就该做的事，只会把每次运行变成点击练习。

这个论点成立，但它**靠两个前提撑着**，第二个现在是缺的：

| 前提 | 现状 |
|---|---|
| 节点职责最小、图可见 | ✅ 成立 |
| **做了什么进事件流、可回放** | ❌ **23 个事件类型零发射**（[DEBT.md](../DEBT.md) B-2），工具调用只留了个计数（[O-4](#二可优化)） |

**「最小职责 + 可审计」里，第二半没兑现的话，最高权限就是没有记录的最高权限。**
所以下面第 4 条不是安全要求，是这个决定自己的前提。

还有一处不在论点覆盖范围内：**主管 AI 不是节点**。
它是编辑时的自由对话，没有「最小职责」这层约束，cwd 又是应用数据目录。
它至少要能回答「刚才它动了什么」。
