...
### R-4 · 一批测试断言的不是它们自称在测的东西

Round 7 点名，逐条查证过：

- **搜索一致性那条**的正则匹配不到真实的防抖代码，于是它对任何实现都绿
- **runs-page 的 tab 用例**直接注入 `selectedId`，`?run=` 这条路径从没被读过
- **防连点那条**串行 await，只走到 state 那一半，ref 那一半没被覆盖
- **`conformance_test` 里的 `continue`** 在夹具形状变化时会静默跳过全部
  validation 夹具 —— 43 组变 0 组，测试照样绿

**验收**：每条都做元测试（改坏实现 → 断言变红）。抓不住退化的断言就删掉，
留着比没有更糟 —— 它占着「这里有覆盖」的位置。

### R-5 · 已批准的确认永不过期，且比对认键序

`claim_approved_confirmation` 拿 `input_json` 做字符串相等比对：
同一份入参，键序不同就认不出（`serde_json::Value` 的 Map 默认按插入序）。
反过来，一次批准过的调用**永远**可以重放 —— 没有 TTL、没有单次消费标记。

**验收**：比对前先规范化（按键排序序列化）；批准有有效期，用过即失效。

### R-6 · FTS 这个子系统要么接上，要么删掉

我上一轮给它加了侧表索引（级联删除从 500 ms 降到 1 ms），
而 Y-6 早就把 `Store::search` 记为死代码 —— 它没有 IPC、没有 MCP、
没有契约出口，界面上搜不到它。**给一个没有出口的子系统做性能优化**
是这一轮最没有价值的一次投入。
...
- **第 7 轮（复核我自己上一轮的修复）**：
  - `delete_model` 引用检查（N-3 余下）+ 界面上那句失效警告改成实话
  - `SessionPool::shutdown` 主动 kill 进程组 —— 上一轮我把「看得见的卡死」
    换成了「看不见的孤儿 node 进程」，注释里还写着 Drop 会兜住（那句在
    「App 退出」这个唯一调用点上是错的）。两条守它的测试都是假的：
    hang mock 从没被调起（整个 suite 0.29 s 跑完），另一条只断言
    `live_count == 0`（`live.clear()` 的同义反复）。现在 10.13 s，
    元测试验过退回旧实现会红
  - `reap_locked` 把「有人在用」与「锁中毒」分开 —— 混着算的话
    中毒的槽位永久坏死且 `live_count` 看不见它
  - **第四道接缝守卫**（`capability_reach_test`）：能力校验里给
    `script.shell` / `script.python` / `git.worktree` 写的三支分支
    **一次都没走到过** —— 它们的 configSchema 里没有 `agentProfileId`，
    而另一条来源 `with_capabilities` 生产里零调用。角色页顶上却写着
    「权限（引擎强制，Prompt 无法越权）」。删掉死分支、界面说清作用范围、
    守卫盯住「别再写够不着的分支」。前三道守「契约声明了引擎没实现」，
    这道守「引擎实现了但够不着」
  - Dry Run 查的 adapter 与执行时用的不是一个 —— 两处各写一套 runtime
    解析、各有一个缺省（claude vs codex）。抽成共用函数，
    守卫直接比对两个 API 的输出而不是各自硬编码期望值
