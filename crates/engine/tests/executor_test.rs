...
fn 角色的目标人设与输出契约都进了提示词() {
    ...
    /*
     * 节点自己的指令仍然要在，而且**排在角色框架之后** ——
     * 它是这一次要干的事，不能被人设埋在中间。
     *
     * 原来断言的是「在最末尾」。多端口节点后面会追加一段
     * 「最后必须写出你的结论走哪个分支」（`choose_ai_port` 那条链路），
     * 那段是**引擎的硬要求**，排在指令后面是对的 ——
     * 它说的是「怎么把结论交出来」，而不是「要干什么」。
     */
    let 指令位置 = prompt.find("看看这段改动").expect("节点指令没进提示词");
    let 角色位置 = prompt.find("你的角色").expect("角色没进提示词");
    assert!(
        指令位置 > 角色位置,
        "节点指令排在了角色框架之前：\n{prompt}"
    );
    assert!(
        prompt.contains("PORT:"),
        "多端口节点要告诉模型怎么表态走哪个分支，否则引擎只能退回第一个：\n{prompt}"
    );
}
