...
VALUES
  ('model:codex', 'Codex（本地 ACP）', 'acp.codex', 'gpt-5-codex', 'high', 400000,
   '["text","tools","structured_output"]', NULL, 1, datetime('now'), datetime('now')),
  ('model:claude', 'Claude Code（本地 ACP）', 'acp.claude', 'claude-opus-5', 'high', 1000000,
   '["text","tools","structured_output","vision"]', NULL, 1, datetime('now'), datetime('now'));
...
VALUES
  ('builtin:analyst', '分析师', '分析',
   ...
   12, 900000, 1, 1, datetime('now'), datetime('now')),
  ...
  ('builtin:builder', '执行者', '执行',
   ...
   24, 1800000, 1, 1, datetime('now'), datetime('now')),
  ...
  ('builtin:operator', '决策者', '决策',
   ...
   8, 600000, 1, 1, datetime('now'), datetime('now'));
...
VALUES
  ...
  ('prompt:review-diff', '审查', 'Diff 审查',
   json_array(
     ...
     json_object('title', 'Task', 'body', '审查 {{target}}，按严重度排序，最多 5 条。'),
     json_object('title', 'Context', 'body', '检查清单：{{checklist}}。'),
     json_object('title', 'Constraints', 'body',
   ...
   json_array(
     json_object('name', 'target', 'required', json('true'), 'description', '要审查的 Diff 或产物'),
     json_object('name', 'checklist', 'required', json('false'), 'description', '额外的检查项')
   ),
   1, 1, datetime('now')),
  ...
  ('prompt:fix-by-plan', '执行', '按方案修复',
   json_array(
     ...
     json_object('title', 'Task', 'body', '按 {{plan}} 修改代码，小步提交。'),
     json_object('title', 'Context', 'body', '工作目录是一个独立的 git worktree，改动不影响主分支。'),
     json_object('title', 'Constraints', 'body',
       '每改一步跑一遍 {{verify}}。红了就地修，不要往下堆。方案没覆盖到的文件先别动。'),
     json_object('title', 'Output contract', 'body', '改了哪些文件、每处为什么改、验证命令与输出。')
   ),
   json_array(
     json_object('name', 'plan', 'required', json('true'), 'description', '选定的方案'),
     json_object('name', 'verify', 'required', json('false'), 'description', '验证命令')
   ),
   1, 1, datetime('now')),
...
--
-- source 是 'builtin'：界面上能看出这条是随应用带来的，
-- 与用户自己写的、AI 写回的区分开。
...
VALUES
  ...
  ('memory:builtin:external-writes', 'workspace', NULL,
   ...
   || '那正是你被派来做的事，放手做。',
   'push / PR / 删除等外部写操作需人工确认；本地改动不算', 'builtin', 'system',
   datetime('now'), datetime('now'), NULL, 'internal', 1,
   json_array('安全'), 1),
  ...
  ('memory:builtin:secrets', 'workspace', NULL,
   ...
   '密钥只用 keychain:// 引用，明文永不落库', 'builtin', 'system',
   datetime('now'), datetime('now'), NULL, 'internal', 1,
   json_array('安全'), 1);
